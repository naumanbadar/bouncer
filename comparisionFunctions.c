...
int tcp_stateInList_wrt_sourceIpAndSourcePortAndDestinationPort(void* data, Node node) {
	...
	if (associationInList->senderSourcePort == associationNew->senderSourcePort
			&& associationInList->senderDestinationPort
					== associationNew->senderDestinationPort
			&& associationInList->senderSourceIp.s_addr
					== associationNew->senderSourceIp.s_addr) {
		return 1;
	} else
		return 0;
}
int tcp_stateInList_wrt_bouncerPortAndIp(void* data, Node node) {
	...
	struct tcp_state * associationNew = (struct tcp_state *) data;
		...
		if (associationNew->senderDestinationPort == associationInList->bouncerSourcePort&&associationNew->senderSourceIp.s_addr==inet_addr(serverIP)) {
